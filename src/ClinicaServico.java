...
public class ClinicaServico {
    ...
    public static Paciente buscarIndicePaciente(String cpf) {
            for (Paciente paciente : pacientes) {
                if (paciente.cpf.equals(cpf)) return paciente;
            }
            return null;
        }
    ...
    if (tipo == 1) {
        ...
        if (esp.equals("fisioterapia")) {     // dados específicos para fisioterapeutas
        System.out.print("Quantidade padrão de sessões: ");
        int sessoes = Integer.parseInt(sc.nextLine());
        
        profissionais[totalProfissionais].setQuantidadeSessoesPadrao(sessoes);
    }
        if (esp.equals("psicologia")) { // dados específicos para psicólogos
        System.out.print("Abordagem terapêutica: ");
        String abordagem = sc.nextLine();

        profissionais[totalProfissionais].setAbordagemTerapeutica(abordagem);
    }
    ...
    public static Profissional buscarIndiceProfissional(String nome) {
        ...
        for (Profissional profissional : profissionais) {
            if (profissional.nome.equals(nome)) return profissional;
        }
        return null;
    }
